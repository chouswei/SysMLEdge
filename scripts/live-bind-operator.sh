...
echo "operator LIVE bind: host=$MEMNET_SERVE_HOST version=$MEMNET_LLM_VERSION proof_pass_claimed=false" >&2
echo "cited Path-B mn_0d4f6178 = published M1 narrow (124+nested) — not this bind session" >&2
echo "#21 FAKE ego ≠ LIVE bind. pin_map ≠ project@rev. H2H not this script." >&2

npx tsx src/cli.ts live-probe
npx tsx src/cli.ts memnet-check
BIND_SMOKE_LIVE=1 bash "$ROOT/scripts/bind-smoke.sh"

echo "proof_pass_claimed=false LIVE_bind_smoke_done=true h2h=not_run cold=not_run" >&2
