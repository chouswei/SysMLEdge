...
export const LIVE_MEMNET_TODO =
  "LIVE M1 Path A on mn_b05a9869 still not bind. Published Path-B narrow mn_0d4f6178 CON=124 + nested qname ego @ 0.19.9+TCP ≠ rev.sha. LIVE bind = SysMLEdge import/reproject. proof_pass_claimed=false until H2H+cold.";
...
export const BIND_SMOKE_THEATER =
  "Kill theater: tip Path-B / pin_map ≠ bind; #21 FAKE ego ≠ LIVE bind; Path A CON=29 on mn_b05a9869 ≠ bind; H2H later scores narrow 124+nested not gold-200. Published Path-B mn_0d4f6178 is not project@rev.";
...
export async function smokeBind(
  ...
  const notes: string[] = [
    BIND_SMOKE_THEATER,
    "proof_pass_claimed=false. scaffold/bind smoke ≠ P1 pass. H2H not run (scores 124+nested later). Kuzu unused.",
  ];
