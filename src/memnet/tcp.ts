...
import { assertSchemaMapFile, defaultSysmlSchemaMapPath, sessionOpenArgs, usesLeftoverMapFlag } from "./schema.js";
...
export class TcpMemNet implements MemNetAdapter {
  ...
  async reproject(ssotDir: string, parsed: ParsedTree): Promise<string | undefined> {
    ...
    const mapFile = assertSchemaMapFile(defaultSysmlSchemaMapPath());
    const openArgs = sessionOpenArgs(mapFile);
    if (usesLeftoverMapFlag(openArgs)) {
      throw new Error("refuse leftover --map wire; MemNet 0.19.9 SCHEMA is --map-file");
    }
    const opened = await this.send(openArgs);
