# RUNLOG — 2026-09-12 — Cut 1 tip **wall apparatus** (Memnetor exact)

**Cite:** Memnetor Cut 1 on Foam **tip** session **`mn_0d4f6178`** @ **0.19.9**. **≠ SysMLEdge bind.** `proof_pass` **not claimed** (`proof_pass=false` / `proof_pass_claimed: false`). **Tip ≠ product.**

Prior scored tip H2H: [RUNLOG-2026-09-12-h2h-foam-tip.md](RUNLOG-2026-09-12-h2h-foam-tip.md) ([#31](https://github.com/chouswei/SysMLEdge/pull/31) wall **FAIL** **0/3** under the **old** tip-wall axis). Cut 1 is the apparatus invent that FAIL opened. **MUST NOT reopen Neo4j.**
...
| Field | Value |
|-------|--------|
| Cut | **1** — tip wall apparatus (warm / depth / Truncation) |
...
`proof_pass` **not claimed**. `mn_0d4f6178` @ **0.19.9**.

| Factor | Memnetor | Close grep wall? |
|--------|----------|:-----------------:|
| **F1** | warm≈cold (~**440ms**); warm **not usefully faster** | **no** |
| **F2** | depth = **bytes not wall** (**+7–21ms**) | **no** |
| **F3** | M grows **bytes not ms**; System Truncation at **M=200** (**omit150**) | **no** |

Arm A grep **~95–106ms** vs B **~400–470ms** (**~4×**).

**Verdict:** wall still **loses badly**; factors **do not explain** the gap.

Honest clip (M=200 omit**150**) ≠ empty / ≠ silent drop.

## Memnetor confirm

**No factor missed inside Cut 1.** The **~400ms floor** is almost **constant** across warm / cold / depth / M → **not** ShapeWalk / Truncation / cold miss.

**Out of scope for Cut 1** (do not treat as missed factors): serve/MCP hop; emit serialize; process-restart cold; session size.
...
MUST NOT treat Cut 1 as row **9** / **M5** product pass.

## Kill theater (do not sell)

- Cut 1 as a wall-clock **win** or as **P1 / M5 / row 9** pass
- Tip wall as a **tip pass/fail** (CEO Core: **log only**)
