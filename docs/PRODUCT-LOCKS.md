...
**H1 (CEO Core 2026-09-12):** `fixtures/memnet-session.map` SCHEMA for PKG/PRT/POR/CON MUST keep `qname` (and `path` / `sysml_kind`). LIVE bind MUST use `--map-file` with those columns. Pi MUST restore the GitHub map — operator-narrow `SCHEMA PRT ; fields=id name kind role status recycle` is a **soft-pass**, not bind. That shape strips locators on `session_save`/`load`. SHA256 CI-pin of the map is **not** this cut (Devicor). `proof_pass_claimed: false`. Not a Foam proof pass.
