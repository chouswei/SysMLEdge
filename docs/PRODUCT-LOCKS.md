...
**H1 (CEO Core 2026-09-12):** `fixtures/memnet-session.map` SCHEMA for PKG/PRT/POR/CON MUST keep `qname` (and `path` / `sysml_kind`). LIVE bind MUST use `--map-file` with those columns. MUST NOT overwrite with narrow `SCHEMA PRT ; fields=id name kind role status recycle` — `session_save`/`load` then drops locators and `pin_map qname=` misses. Merge ingest columns (`partNumber`, CON `kind`) with locators; do not drop either side. `proof_pass_claimed: false`. Not a Foam proof pass.
