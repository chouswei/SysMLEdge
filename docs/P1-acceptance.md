...
5. Timed **bind** H2H wall-clock + context on **0.19.8+TCP after bind** (**M5** / row **9**) on Foam bind desk **`mn_c7b75f2c`**: **FAIL** (context **1/3**, no_drop **3/3**, wall **0/3** P/F). M1 cite remains `mn_0d4f6178` (CON **124** + nested). Tip ≠ this desk. Gold-200 stays out. H2H on **p1-tiny** / `mn_be03c1a9` = **plumbing only**. MUST NOT soft-pass the wedge. **Track B executed.** Tip wall is **not** this step.
...
| Tiny / `mn_be03c1a9` H2H sold as wedge pass | Stop. Plumbing only. Product bind H2H + cold **FAIL** on `mn_c7b75f2c`. |
...
**P1 head-to-head** scores **wall-clock + context footprint + no silent drop** (pains 1–2). **CEO / Elon / Edison Core:** Timed H2H must score the **narrow** claim (CON **124** + nested ego; M1 cite `mn_0d4f6178`). H2H on p1-tiny / `mn_be03c1a9` = **plumbing only**. Product bind desk **`mn_c7b75f2c`**: **FAIL**. MUST NOT soft-pass the wedge. Gold-200 stays out of the competitor story. Pains **3–5** are proven by STALE / propose / ship-rev / reproject rows below — **not** extra feature work this week.

**CEO Core — narrow tip (2026-09-12):** Tip **must-win** = **context + no silent drop** only. Tip **wall-clock** = **log only** (not a tip pass). Product **bind** H2H (row **9** / **M5**) still logs wall as **P/F**. **Track B executed** — product FAIL. **No Neo4j.** `proof_pass_claimed: false`.
...
**Honesty — Foam bind H2H + cold (Devicor stated, 2026-09-12):** `mn_c7b75f2c` @ `rev.sha` `f4679d848ccab5b26875eb9d2812171bb759ebf1`. Bind invent **GREEN**. `must_win=false`. context **1/3**. no_drop **3/3**. wall **0/3** (P/F). cold **FAIL**. No per-Q ms in-repo. MUST NOT invent ms. MUST NOT claim faster-than-grep. Bind GREEN ≠ product pass.

| # | Name | Pass criteria |
|---|------|----------------|
| 9 | Head-to-head | **FAIL** on Foam bind `mn_c7b75f2c` (context **1/3**, wall **0/3** P/F; no_drop **3/3**). Bar remains **wall-clock + context footprint + no silent drop** on the three Foam Qs, scoring the **narrow** claim (M1 cite `mn_0d4f6178`). Competitor = tens-of-minutes grep, not Cameo, **not** gold-200. H2H on **p1-tiny** / `mn_be03c1a9` = **plumbing only**. |
...
| 12 | Non-Core operator | **FAIL.** One operator **not** from Core runs the demo **cold**. Keep using this vs grep? **Yes/no + why.** Cold **no** → kill the wedge claim. |
...
| **M5** | Wall-clock + context vs grep | **FAIL** on Foam bind `mn_c7b75f2c` @ `f4679d848ccab5b26875eb9d2812171bb759ebf1` (`must_win=false`: context **1/3**, no_drop **3/3**, wall **0/3** P/F). No per-Q ms in this fold. MUST NOT invent ms. MUST NOT claim faster-than-grep. Bind GREEN ≠ product pass. [proof/RUNLOG-2026-09-12-h2h-foam-bind.md](proof/RUNLOG-2026-09-12-h2h-foam-bind.md). M1 cite remains `mn_0d4f6178`. H2H on **p1-tiny** / `mn_be03c1a9` = **plumbing only**. **Honesty (tip Track A, [#31](https://github.com/chouswei/SysMLEdge/pull/31), Devicor ms):** must-win **PASS**; wall **FAIL** **0/3** log-only — **≠** this M5 product bar. CLI `head-to-head` is a **null scaffold**. Gold-200 stays out. Kill: tip Path-B sold as bind; H2H vs gold-200; tiny soft-pass; bind GREEN as pass. |
