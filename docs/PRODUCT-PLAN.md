...
**Invent status:** mirror-lie **PASS** (InventProbe in MemNet; gold parts **11→13**, ports **12→13**; zip≡disk). Same-session re-ingest **appends** (rows **56→116**, dup qnames) — honesty. **Lock (f)** (user GO 2026-09-12 + Memnetor): reproject opens a **new** MemNet session id (ops-only). **(r)** deferred; no MemNet API change this cut. H2H after a clean **(f)** path. LIVE bind `mn_27ce8714` is **p1-tiny** (CON \|Q\|=**5**) — **no M1 claim**. `proof_pass_claimed: false`. don’t-migrate / no Pro until cold Foam. KEEP MemNet sole. Do not reopen Neo4j.
